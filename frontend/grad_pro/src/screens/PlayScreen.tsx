...
const LyricMakeScreen = ({ route, navigation }) => {
  ...
  const { userId, requestData, type } = route.params || {}; // route.params 기본값을 빈 객체로 설정

  // requestData가 없을 때 기본값 사용
  ...
  if (type === 'Play' && !requestData) {
    return (
      <View style={[BaseStyles.flexContainer, { backgroundColor: '#A5BEDF' }]}>
        <Header />
        <View style={[BaseStyles.contentContainer, { justifyContent: 'center', alignItems: 'center' }]}>
          <Text style={[BaseStyles.mainText, { fontSize: 25, color: '#000' }]}>아직 동요가 없어요</Text>
        </View>
      </View>
    );
  }
